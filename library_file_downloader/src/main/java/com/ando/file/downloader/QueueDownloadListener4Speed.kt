...
import com.ando.file.downloader.QueueController4Speed.Companion.STATUS_LOOK_OVER
...
class QueueDownloadListener4Speed : DownloadListener4WithSpeed() {
    ...
    fun resetInfo(task: DownloadTask, holder: FileBaseViewHolder) {
        ...
        // task name
        val taskName = com.ando.file.downloader.QueueTagUtils.getTaskName(task)
        holder.setText(R.id.tv_down_name, taskName)

        // process references
        val status = com.ando.file.downloader.QueueTagUtils.getStatus(task)
        ...
        if (status != null) {
            ...
            if (status == EndCause.COMPLETED.name) {
                ...
                val total = com.ando.file.downloader.QueueTagUtils.getTotal(task)
                FileLogger.e(
                    TAG, "total= $total   ${com.ando.file.downloader.QueueTagUtils.getOffset(
                        task
                    )}"
                )
                if (total == 0L) {
                    ...
                    DownloadProgressUtils.calcProgressToViewAndMark(
                        progressBar,
                        com.ando.file.downloader.QueueTagUtils.getOffset(task), total, false
                    )
                }
            }
            ...
            com.ando.file.downloader.QueueTagUtils.saveStatus(
                task,
            ...
            if (statusOnStore == StatusUtil.Status.COMPLETED) {
                ...
                if (statusOnStore == StatusUtil.Status.UNKNOWN) {
                    ...
                    if (info != null) {
                        ...
                        com.ando.file.downloader.QueueTagUtils.saveTotal(
                            task,
                            info.totalLength
                        )
                        com.ando.file.downloader.QueueTagUtils.saveOffset(
                            task,
                            info.totalOffset
                        )
                        //向 ProgressBar 设置进度
    ...
    override fun taskStart(task: DownloadTask) {
        ...
        com.ando.file.downloader.QueueTagUtils.saveStatus(task, status)
...
//        val percent = currentOffset.toFloat() / totalLength * 100
        ...
        val status = DownloadListener1Status.PROGRESS
        com.ando.file.downloader.QueueTagUtils.saveStatus(task, status)
        com.ando.file.downloader.QueueTagUtils.saveOffset(task, currentOffset)

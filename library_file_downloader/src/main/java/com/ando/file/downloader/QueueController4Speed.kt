...
import com.ando.file.downloader.view.FileRecyclerAdapter
...
class QueueController4Speed {
    ...
    private val listener = com.ando.file.downloader.QueueDownloadListener4Speed()
    ...
    fun initTasks(
        ...
        for (taskBean in urlList) {
            ...
            //注:断点下载 根据的是 任务名
            //断点续传失效问题
            com.ando.file.downloader.QueueTagUtils.saveTaskName(
                boundTask,
                taskBean.tname
            )
    ...
    fun deleteFiles() {
        ...
        for (task in taskList) {
            com.ando.file.downloader.QueueTagUtils.clearProceedTask(task)
        }
    ...
    fun deleteFile(task: DownloadTask) {
        ...
        for (t in taskList) {
            if (t == task) {
                com.ando.file.downloader.QueueTagUtils.clearProceedTask(t)
                break
            }
        }
    ...
    fun setPriority(task: DownloadTask, priority: Int) {
        ...
        com.ando.file.downloader.QueueTagUtils.savePriority(newTask, priority)
    }

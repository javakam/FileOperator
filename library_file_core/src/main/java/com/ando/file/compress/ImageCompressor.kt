...
class ImageCompressor private constructor(builder: Builder) : Handler.Callback {
    ...
    private operator fun get(uri: Uri, context: Context): Uri? =
        ImageCompressEngine.compressCompat(uri, getImageCacheFile(context, ImageChecker.extSuffix(uri)), cache, focusAlpha)
    ...
    private fun compressReal(context: Context, uri: Uri): Uri? {
        ...
        return if (mImageCompressPredicate != null) {
            ...
            if (ImageChecker.needCompress(mIgnoreMinCompressSize, uri)) ImageCompressEngine.compressCompat(uri, targetFile, cache, focusAlpha) else uri
        }
    }
